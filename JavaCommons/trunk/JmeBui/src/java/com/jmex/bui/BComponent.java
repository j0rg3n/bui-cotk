...
{
	/**
	 * The default component state. This is used to select the component's style
	 * pseudoclass among other things.
	 */
	public static final int DEFAULT = 0;
	/**
	 * A component state indicating that the mouse is hovering over the
	 * component. This is used to select the component's style pseudoclass among
	 * other things.
	 */
	public static final int HOVER = 1;
	/**
	 * A component state indicating that the component is disabled. This is used
	 * to select the component's style pseudoclass among other things.
	 */
	...
	public static void applyDefaultStates()
	{
		...
		{
			if (Renderer.defaultStateList[ii] != null && Renderer.defaultStateList[ii] != Renderer.getCurrentState(ii))
			{
	...
	public void setStyleClass(String styleClass)
	{
		if (isAdded())
		{
			System.err.println("Warning: attempt to set style class after " + "component was added to the interface " + "heirarchy [comp=" + this + "].");
			Thread.dumpStack();
		}
	...
	public void setParent(BComponent parent)
	{
		...
		{
			Log.log.warning("Already added child readded to interface " + "hierarchy! [comp=" + this + ", oparent=" + _parent + ", nparent=" + parent + "].");
			Thread.dumpStack();
		}
		else if (_parent == null && parent == null)
		{
			Log.log.warning("Already removed child reremoved from interface " + "hierarchy! [comp=" + this + "].");
			Thread.dumpStack();
		}
		_parent = parent;
	}
	 ...
	 * two dimensions. Not all components will make use of the hints, but layout
	 * managers should provide them if they know the component will be forced to
	 * a particular width or height regardless of what it prefers.
	 */
	...
	/**
	 * Configures the preferred size of this component. This will override any
	 * information provided by derived classes that have opinions about their
	 * preferred size.
	 */
	 ...
	 * never be returned, an {@link Insets} instance with all fields set to zero
	 ...
	 * should not respond to user interaction and should render itself in such a
	 * way as not to afford user interaction.
	 */
	...
	/**
	 * Sets a user defined property on this component. User defined properties
	 * allow the association of arbitrary additional data with a component for
	...
	/**
	 * Returns the component that should receive focus if this component is
	 * clicked. If this component does not accept focus, its parent will be
	 * checked and so on.
	 */
	...
	/**
	 * Adds a listener to this component. The listener will be notified when
	 * events of the appropriate type are dispatched on this component.
	 */
	...
	/**
	 * Removes a listener from this component. Returns true if the listener was
	 * in fact in the listener list for this component, false if not.
	 */
	 ...
	 * @return
	 */
	public ComponentListener getListener(int i)
	{
		if (_listeners != null)
		{
			return _listeners.get(i);
		}
		else
		{
			return null;
	 ...
	 * &lt;html&gt; then the tooltip will be displayed with an
	 * 
	 * @{link HTMLView} otherwise it will be displayed with a {@link BLabel}.
	 */
	...
	/**
	 * Returns true if this component is added to a hierarchy of components that
	 * culminates in a top-level window.
	 */
	...
	/**
	 * Marks this component as invalid and needing a relayout. If the component
	 * is valid, its parent will also be marked as invalid.
	 */
	 ...
	 * and border and then calls {@link #renderComponent} to allow the component
	 * to render itself.
	 */
	...
	/**
	 * Returns the component "hit" by the specified mouse coordinates which
	 * might be this component or any of its children. This method should return
	 * null if the supplied mouse coordinates are outside the bounds of this
	 * component.
	 */
	 ...
	 * <code>super.dispatchEvent</code> for events that they did not
	 * "consume".
	 * 
	...
	/**
	 * This method is called when we are added to a hierarchy that is connected
	 * to a top-level window (at which point we can rely on having a look and
	 * feel and can set ourselves up).
	 */
	 ...
	 * connected to a top-level window. If we wish to clean up after things done
	 * in {@link #wasAdded}, this is a fine place to do so.
	 */
	...
	/**
	 * Returns the default stylesheet class to be used for all instances of this
	 * component. Derived classes will likely want to override this method and
	 * set up a default class for their type of component.
	 */
	 ...
	 * These states correspond to stylesheet pseudoclasses that allow components
	 * to customize their configuration based on whether they are enabled or
	 * disabled, or pressed if they are a button, etc.
	 */
	 ...
	 * If such a component can be found, it will be returned. If no other
	 * focusable component can be found and this component is focusable, this
	 * component will be returned. Otherwise, null will be returned.
	 */
	...
	/**
	 * Searches for the previous component that should receive the keyboard
	 * focus. If such a component can be found, it will be returned. If no other
	 * focusable component can be found and this component is focusable, this
	 * component will be returned. Otherwise, null will be returned.
	 */
	 ...
	 * root node for processing though in general it will result in an immediate
	 * call to {@link #dispatchEvent} with the event.
	 * 
	 * @return true if the event was emitted, false if it was dropped because we
	 *         are not currently added to the interface hierarchy.
	 */
	protected boolean emitEvent(BEvent event)
	{
		BWindow window;
		BRootNode node;
		if ((window = getWindow()) == null || (node = window.getRootNode()) == null)
		{
			return false;
		}
