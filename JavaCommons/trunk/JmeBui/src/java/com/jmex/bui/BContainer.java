...
{
	/**
	 * Creates a container with no layout manager. One should subsequently be
	 * set via a call to {@link #setLayoutManager}.
	 */
	public BContainer()
	{
	}
	...
	/**
	 * Returns the next component that should receive focus in this container
	 * given the current focus owner. If the supplied current focus owner is
	 * null, the container should return its first focusable component. If the
	 * container has no focusable components following the current focus, it
	 * should call {@link #getNextFocus()} to search further up the hierarchy.
	 */
	 ...
	 * container given the current focus owner. If the supplied current focus
	 * owner is null, the container should return its last focusable component.
	 * If the container has no focusable components before the current focus, it
	 * should call {@link #getPreviousFocus()} to search further up the
	 * hierarchy.
	 */
	...
	protected void applyOperation(ChildOp op)
	{
		for (int ii = 0, ll = getComponentCount(); ii < ll && ii < getComponentCount(); ii++)
		{
			...
			{
				Log.log.log(Level.WARNING, "Child operation choked" + "[op=" + op + ", child=" + child + "].", e);
			}
		}
	}
