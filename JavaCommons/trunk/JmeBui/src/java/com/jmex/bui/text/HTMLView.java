 ...
 * 
 * <p>
 * Note: width and height hints do not work. The Java HTML code doesn't seem to
 * handle being sized to a particular width or height and then determining its
 * preferred span along the other axis. So we always get the "natural" preferred
 * size of the HTML without any forced wrapping. Of course if the component is
 * forcibly made smaller, the HTML will be wrapped, but it may not fit in the
 * vertical or horizontal space made available. Caveat user.
 */
public class HTMLView extends BComponent
{
	/**
	 * Creates a blank HTML view. The HTML contents can be set later with a call
	 * to {@link #setContents}.
	 */
	public HTMLView()
	{
	...
	public void setStyleSheet(String stylesheet)
	{
		...
		{
			Log.log.log(Level.WARNING, "Failed to parse stylesheet " + "[sheet=" + stylesheet + "].", t);
		}
	}
	...
	public void setContents(String contents)
	{
		...
		{
			Log.log.log(Level.WARNING, "Failed to parse HTML " + "[contents=" + contents + "].", t);
		}
	}
	...
	{
		_view = new BridgeView(_kit.getViewFactory().create(document.getDefaultRootElement()));
		forceRelayout();
	}
