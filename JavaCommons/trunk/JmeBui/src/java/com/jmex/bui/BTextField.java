 ...
 */
public class BTextField extends BTextComponent implements EditCommands, Document.Listener
{
	/**
	...
	public boolean dispatchEvent(BEvent event)
	{
		if (event instanceof KeyEvent)
		{
			...
			if (kev.getType() == KeyEvent.KEY_PRESSED)
			{
				...
				default:
					...
					if ((modifiers & ~InputEvent.SHIFT_DOWN_MASK) == 0 && !Character.isISOControl(c))
					{
		...
		else if (event instanceof MouseEvent)
		{
			...
			_text.getLength() > 0)
			{
				...
				int mx = mev.getX() - getAbsoluteX() - insets.left + _txoff, my = mev.getY() - getAbsoluteY() - insets.bottom;
	...
	protected void renderComponent(Renderer renderer)
	{
		...
		if (_glyphs != null)
		{
			...
			GL11.glScissor(getAbsoluteX() + insets.left, getAbsoluteY() + insets.bottom, _width - insets.getHorizontal(), _height - insets.getVertical());
			try
			{
	...
	{
		Dimension d = (_glyphs == null) ? new Dimension(0, getTextFactory().getHeight()) : new Dimension(_glyphs.getSize());
		if (_prefWidth != -1)
		{
			d.width = _prefWidth;
		}
	...
	protected void recreateGlyphs()
	{
		...
		_glyphs = getTextFactory().createText(getDisplayText(), getColor(), BConstants.NORMAL, null, true);
		if (isAdded())
		{
			_glyphs.wasAdded();
		}
