...
public class AWTTextFactory extends BTextFactory
{
	...
	// documentation inherited
	@Override
	public BText createText(String text, ColorRGBA color, int effect, ColorRGBA effectColor, boolean useAdvance)
	{
		if (text.equals(""))
		{
			text = " ";
		...
		try
		{
			...
			layout = new TextLayout(parseStyledText(text, _attrs, null).getIterator(), gfx.getFontRenderContext());
		}
		finally
		{
			gfx.dispose();
		}
		return createText(text, layout, color, effect, effectColor, text.length(), useAdvance);
	}

	// documentation inherited
	@Override
	public BText[] wrapText(String text, ColorRGBA color, int effect, ColorRGBA effectColor, int maxWidth)
	{
		...
		try
		{
			...
			while (pos < text.length())
			{
				...
				texts.add(createText(text.substring(unmodpos - length, unmodpos), layout, color, effect, effectColor, length, true));
			}
		}
		finally
		{
	...
	/** Helper function. */
	protected BText createText(final String origtext, final TextLayout layout, ColorRGBA color, final int effect, ColorRGBA effectColor, final int length, boolean useAdvance)
	{
		...
		return new BText()
		{
			...
			/*
			 * public void release() { bimage.release(); }
			 */
		};
	}
	 ...
	 */
	protected AttributedString parseStyledText(String text, HashMap<TextAttribute, Font> attrs, String[] bare)
	{
		...
		for (int ii = 0, ll = text.length(); ii < ll; ii++)
		{
			...
			if (parenidx == -1)
			{
				Log.log.warning("Invalid style specification, missing paren " + "[text=" + text + ", pos=" + ii + "].");
				continue;
			}
			...
			for (int ss = 0, ssl = styles.length(); ss < ssl; ss++)
			{
				...
				if (run.styles[ss] == '#')
				{
					if (ss > ssl - 7)
					{
						Log.log.warning("Invalid color definition " + "[text=" + text + ", color=" + styles.substring(ss) + "].");
						ss = ssl;
					}
					else
					{
						...
						{
							Log.log.warning("Invalid color definition " + "[text=" + text + ", color=#" + hex + "].");
						}
					}
				}
			}
		...
		for (int ii = 0; ii < runs.size(); ii++)
		{
			...
			for (int ss = 0; ss < run.styles.length; ss++)
			{
				...
				case 0: // ignore blank spots
					break;
				default:
					Log.log.warning("Invalid style command [text=" + text + ", command=" + run.styles[ss] + ", run=" + run + "].");
					break;
				}
			}
		}
