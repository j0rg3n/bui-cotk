...
public class BComboBox extends BLabel
{
	...
	public boolean dispatchEvent(BEvent event)
	{
		if (event instanceof MouseEvent)
		{
			...
			case MouseEvent.MOUSE_PRESSED:
				if (_menu == null)
				{
					_menu = new BPopupMenu(getWindow(), false, getWidth());
					_menu.setLayer(Integer.MAX_VALUE);
				...
				System.out.println("CREATING MENU !!!:" + getAbsoluteX() + "," + getAbsoluteY());
	...
	protected ActionListener _listener = new ActionListener()
	{
		...
		{
			selectItem(_items.indexOf(event.getSource()), event.getWhen(), event.getModifiers());
		}
	};
