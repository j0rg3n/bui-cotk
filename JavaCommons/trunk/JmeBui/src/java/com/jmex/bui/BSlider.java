 ...
 */
public class BSlider extends BComponent implements BConstants
{
	/**
	...
	/**
	 * Returns the value of the slider (calculated from its position) in the
	 * range [0-1].
	 * 
	 * @return
	 */
	...
	protected void updateValue(int mx, int my)
	{
		...
		if (_orient == HORIZONTAL)
		{
			...
			float baseval = Math.round((mx - fwid / 2) * (float) _model.getRange() / (getWidth() - insets.getHorizontal() - fwid));
			/*
			 * This will make it more sticky float baseval = (mx - fwid/2) *
			 * (float)_model.getRange() / (getWidth() - insets.getHorizontal() -
			 * fwid); if(baseval + _model.getMinimum() > _model.getValue()) {
			 * baseval = (float) Math.floor(baseval); } else { baseval = (float)
			 * Math.ceil(baseval); }
			 */
		...
		else
		{
			...
			float baseval = Math.round((my - fhei / 2) * _model.getRange() / (getHeight() - insets.getVertical() - fhei));
			_model.setValue(_model.getMinimum() + (int) baseval);
		}
	}
