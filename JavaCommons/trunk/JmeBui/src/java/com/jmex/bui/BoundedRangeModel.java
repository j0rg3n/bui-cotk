...
/**
 * Defines the model used by the {@link BScrollBar} to communicate with other
 * components and external entities that wish to be manipulated by a scroll bar.
 * 
 * <p>
 * A bounded range model has a minimum and maximum value, a current value and an
 * extent. These are easily visualized by showing how they control a scroll bar:
 * 
 * <pre>
 *  +-------------------------------------------------------------------+
 *  |        +---------------------------------------+                  |
...
{
	/**
	 * Creates a bounded range model with the specified minimum value, current
	 * value, extent and maximum value.
	 */
	...
	/**
	 * Configures the minimum value of this model, adjusting the value, extent
	 * and maximum as necessary to maintain the consistency of the model.
	 */
	...
	/**
	 * Configures the maximum value of this model, adjusting the value, extent
	 * and minimum as necessary to maintain the consistency of the model.
	 */
	 ...
	 * does not fall within the range of <code>min <= value <= max -
	 * extent<code>.
	 */
	...
	/**
	 * Configures the extent of this model. The new value will be adjusted if it
	 * does not fall within the range of <code>0 <= extent <= max -
	 * value<code>.
	 */
	...
	/**
	 * Configures this model with a new minimum, maximum, current value and
	 * extent.
	 * 
	 * @return true if the range was modified, false if the values were already
	 *         set to the requested values.
	 */
	public boolean setRange(int min, int value, int extent, int max)
	{
		...
		value = Math.max(min, Math.min(max - extent, value));
