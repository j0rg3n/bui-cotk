...
public class BWindow extends BContainer implements Comparable
{
	...
	/**
	 * Sizes this window to its preferred size. This method does not change the
	...
	/**
	 * Positions this window in the center of the display. This should be called
	 * after configuring the size of the window (using, for example, a call to
	 * {@link #pack}).
	 */
	public void center()
	{
	...
	/**
	 * Configures this window to be modal which causes it to "steal" all mouse
	 * and keyboard input while it is added to the interface hierarchy. Mouse
	 * movement and button press events that would normally go to other windows
	 * or to the default mouse target will instead be sent to the top-most modal
	 * window. Other events destined for other windows (key events and mouse
	...
	/**
	 * Returns whether or not this window is modal. See {@link #setModal} for
	 * more information on modality.
	 */
	...
	/**
	 * Returns the layer occupied by this window. See {@link #setLayer} for more
	 * details.
	 */
	public int getLayer()
	{
	 ...
	 * one atop another with the most recently added window being highest in the
	 * stack. The layer configuration allows a window to be added above windows
	 * in a lower layer regardless of when it or other windows are added. All
	 * windows default to a layer of zero, windows with a higher layer will be
	 * "above" those with a lower layer. Windows in the same layer stack
	...
	/**
	 * Configures this window with its root node. Do not call this method, it is
	 * called automatically when a window is added to the root node via a call
	 * to {@link BRootNode#addWindow}.
	 */
	...
	/**
	 * Whether or not this window steals all input from other windows further
	 * down the hierarchy.
	 */
	...
	/**
	 * Used to store a reference to our focus when this window is no longer the
	 * top-most window.
	 */
