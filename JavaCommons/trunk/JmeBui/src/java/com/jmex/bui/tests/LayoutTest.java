...
public class LayoutTest extends BaseTest
{
	...
	protected void createWindows(final BRootNode root, BStyleSheet style)
	{
		...
		try
		{
			icon = new BImage(getClass().getClassLoader().getResource("rsrc/textures/scroll_up.png"));
		}
		catch (Exception e)
		{
		...
		button.setTooltipText("This is a very long tooltip the likes of " + "which you may not dare to contemplate. Indeed " + "it is so long that I expect it to wrap.");
		...
		_text = new BTextArea();
		_slider = new BSlider(BSlider.VERTICAL, 0, 100, 0);
		_slider.addListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
				int extratext = _text.getScrollModel().getMaximum() - _text.getScrollModel().getExtent();
				_text.getScrollModel().setValue((int) (extratext - _slider.getSliderValue() * extratext));
			}
		});
		window = new BWindow(style, new BorderLayout(5, 5));
		window.add(_slider, BorderLayout.WEST);
		window.add(_text, BorderLayout.CENTER);
		window.add(_input = new BTextField(), BorderLayout.SOUTH);
		window.add(new BScrollBar(BScrollBar.VERTICAL, _text.getScrollModel()), BorderLayout.EAST);
		window.add(new BScrollBar(BScrollBar.HORIZONTAL, 0, 25, 50, 100), BorderLayout.NORTH);
		...
		window.add(new BSlider(BSlider.HORIZONTAL, 0, 100, 25), BorderLayout.SOUTH);
		root.addWindow(window);
		window.pack();
