...
/**
 * Processes the polled input information available from the underlying input
 * system into input events and dispatches those to the appropriate parties.
 */
public class PolledRootNode extends BRootNode
{
	private static final long serialVersionUID = -4071568273235426594L;
	...
	public void updateWorldData(float timePerFrame)
	{
		...
		if (_pressed >= 0 && _nextRepeat < _tickStamp)
		{
			...
			else
			{
				...
				KeyEvent event = new KeyEvent(PolledRootNode.this, _tickStamp, _modifiers, KeyEvent.KEY_PRESSED, _presschar, _pressed);
	...
	protected KeyInputListener _keyListener = new KeyInputListener()
	{
		public void onKey(char character, int keyCode, boolean pressed)
		{
			...
			KeyEvent event = new KeyEvent(PolledRootNode.this, _tickStamp, _modifiers, pressed ? KeyEvent.KEY_PRESSED : KeyEvent.KEY_RELEASED, character, keyCode);
			dispatchEvent(_focus, event);
	...
	protected MouseInputListener _mouseListener = new MouseInputListener()
	{
		public void onButton(int button, boolean pressed, int x, int y)
		{
			...
			dispatchEvent(new MouseEvent(PolledRootNode.this, _tickStamp, _modifiers, pressed ? MouseEvent.MOUSE_PRESSED : MouseEvent.MOUSE_RELEASED, button, x, y));
		...
		public void onMove(int xDelta, int yDelta, int newX, int newY)
		{
			...
			dispatchEvent(new MouseEvent(PolledRootNode.this, _tickStamp, _modifiers, _ccomponent != null ? MouseEvent.MOUSE_DRAGGED : MouseEvent.MOUSE_MOVED, newX, newY));
		}

		public void onWheel(int wheelDelta, int x, int y)
		{
			dispatchEvent(new MouseEvent(PolledRootNode.this, _tickStamp, _modifiers, MouseEvent.MOUSE_WHEELED, -1, x, y, wheelDelta));
		}

		protected void dispatchEvent(MouseEvent event)
		{
			PolledRootNode.this.dispatchEvent(_ccomponent != null ? _ccomponent : _hcomponent, event);
		}
	};
	...
	protected static final int[] KEY_MODIFIER_MAP = { KeyInput.KEY_LSHIFT, InputEvent.SHIFT_DOWN_MASK, KeyInput.KEY_RSHIFT, InputEvent.SHIFT_DOWN_MASK, KeyInput.KEY_LCONTROL, InputEvent.CTRL_DOWN_MASK, KeyInput.KEY_RCONTROL, InputEvent.CTRL_DOWN_MASK, KeyInput.KEY_LMENU, InputEvent.ALT_DOWN_MASK, KeyInput.KEY_RMENU, InputEvent.ALT_DOWN_MASK, KeyInput.KEY_LWIN, InputEvent.META_DOWN_MASK, KeyInput.KEY_RWIN, InputEvent.META_DOWN_MASK, };
