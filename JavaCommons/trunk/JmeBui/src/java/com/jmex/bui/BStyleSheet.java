 ...
 * textfield, component, popupmenu, etc.</code>
 * The component's stylesheet class can be overridden with a call to
 * {@link BComponent#setStyleClass}.
 * 
 * <p>
 ...
 * 
 * <p>
 * This resolution process is followed at the time the component is added to the
 * interface hierarchy and the result is used to configure the component. We
 * tradeoff the relative expense of doing the lookup every time the component is
 * rendered (every frame) with the memory expense of storing the style of every
 * component in memory.
 */
public class BStyleSheet
{
	...
	public static void main(String[] args)
	{
		...
		try
		{
			style = new BStyleSheet(new InputStreamReader(BStyleSheet.class.getClassLoader().getResourceAsStream("rsrc/style.bss")), new DefaultResourceProvider());
		}
		catch (Exception e)
		{
	 ...
	 */
	public BStyleSheet(Reader reader, ResourceProvider rsrcprov) throws IOException
	{
		_rsrcprov = rsrcprov;
	...
	protected Rule startRule(StreamTokenizer tok) throws IOException
	{
	...
	protected boolean parseProperty(StreamTokenizer tok, Rule rule) throws IOException
	{
		if (tok.nextToken() == '}')
		{
			return false;
		...
		while (tok.nextToken() != ';' && tok.ttype != '}')
		{
			...
			case StreamTokenizer.TT_NUMBER:
				...
				break;
			default:
				System.err.println("Unexpected token: '" + (char) tok.ttype + "'. Line " + tok.lineno() + ".");
				break;
			}
		}
		try
		...
		{
			System.err.println("Failure parsing property '" + name + "' line " + sline + ": " + e.getMessage());
	...
	protected Object createProperty(String name, ArrayList<Comparable> args)
	{
		...
		else if (name.equals("background"))
		{
			...
			else if (bprop.type.equals("image"))
			{
				...
				if (args.size() > 2)
				{
					...
					if (scval == null)
					{
						throw new IllegalArgumentException("Unknown background scaling type: '" + scale + "'");
					}
			...
			{
				throw new IllegalArgumentException("Unknown background type: '" + bprop.type + "'");
			}
			return bprop;
		}
		...
		else if (name.equals("font"))
		{
			try
			{
				...
				if (!fprop.style.equals(PLAIN) && !fprop.style.equals(BOLD) && !fprop.style.equals(ITALIC) && !fprop.style.equals(BOLD_ITALIC))
				{
			...
			{
				e.printStackTrace(System.err);
				throw new IllegalArgumentException("Fonts must be specified as: " + "\"Font name\" plain|bold|italic|bolditalic point-size");
			}
		}
	...
	protected void fail(StreamTokenizer tok, String expected) throws IOException
	{
		String err = "Parse failure line: " + tok.lineno() + " expected: '" + expected + "' found: '";
		switch (tok.ttype)
		{
		case StreamTokenizer.TT_WORD:
			err += tok.sval;
	...
	protected static class Rule
	{
		...
		public Object get(HashMap<String, Rule> rules, String key)
		{
