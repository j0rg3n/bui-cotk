...
public class JMEBitmapTextFactory extends BTextFactory
{
	...
	public JMEBitmapTextFactory(URL font, int width, int height)
	{
		...
		Texture texture = TextureManager.loadTexture(font, Texture.MM_NONE, Texture.FM_NEAREST);
		...
		// background
		_astate = DisplaySystem.getDisplaySystem().getRenderer().createAlphaState();
	...
	// documentation inherited
	@Override
	public BText createText(final String text, final ColorRGBA color, int effect, ColorRGBA effectColor, boolean useAdvance)
	{
		...
		tgeom.setCullMode(SceneElement.CULL_NEVER);
