...
public class BScrollBar extends BContainer implements BConstants
{
	...
	/**
	 * Creates a scroll bar with the specified orientation, range, value and
	 * extent.
	 */
	...
	protected void update()
	{
		...
		_thumb.setBounds(_well.getX() + winsets.left + tx, _well.getY() + winsets.bottom + ty, twidth, theight);
	}
	...
	protected MouseListener _wellListener = new MouseAdapter()
	{
		...
		public void mousePressed(MouseEvent event)
		{
			...
			int mx = event.getX() - getAbsoluteX(), my = event.getY() - getAbsoluteY(), dv = 0;
			if (_orient == HORIZONTAL)
			{
				if (mx < _thumb.getX())
				{
	...
	protected MouseAdapter _thumbListener = new MouseAdapter()
	{
		...
		public void mouseDragged(MouseEvent event)
		{
			...
			if (_orient == HORIZONTAL)
			{
				...
				dv = (mx - _sx) * _model.getRange() / (_well.getWidth() - _well.getInsets().getHorizontal());
			}
			else
			{
