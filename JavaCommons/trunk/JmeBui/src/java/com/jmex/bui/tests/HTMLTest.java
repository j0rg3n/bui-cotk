...
public class HTMLTest extends BaseTest
{
	...
	protected void createWindows(BRootNode root, BStyleSheet style)
	{
		...
		{
			private static final long serialVersionUID = -1808484867154372428L;
			...
			public Font getFont(AttributeSet attrs)
			{
				...
				if (variant != null && variant.equalsIgnoreCase("Test"))
				{
					...
					if (StyleConstants.isSuperscript(attrs) || StyleConstants.isSubscript(attrs))
					{
						size -= 2;
					}
					return new Font("Serif", style1, size);
				}
				else
				{
		...
		try
		{
			String styledef = ".test { " + "font-variant: \"Test\"; " + "font-size: 16; }";
		...
		view.setContents("<html><body>" + "<span class=\"test\">This is some test <b>HTML!</b></span> " + "With a fairly long first line that we might even wrap." + "<p><table border=1 width=\"100%\">" + "<tr><td>We</td><td>even</td><td>support</td></tr>" + "<tr><td colspan=3 align=center><i>tables!</i></td></tr></table>" + "</body></html>");
