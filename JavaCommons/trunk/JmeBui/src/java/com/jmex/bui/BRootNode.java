...
public abstract class BRootNode extends Geometry
{
	...
	{
		return (_windows.size() > 0 && _windows.get(_windows.size() - 1) == window);
	}
	...
	/**
	 * Requests that the specified component be given the input focus. Pass null
	 * to clear the focus.
	 */
	 ...
	 * component in the normal course of events can call this method to force an
	 * update to the tooltip window.
	 */
	...
	public void updateGeometricState(float time, boolean initiator)
	{
		...
		_lastMoveTime += time;
		String tiptext;
		if (_hcomponent == null || _tipwin != null || _lastMoveTime < getTooltipTimeout() || (tiptext = _hcomponent.getTooltipText()) == null)
		{
			return;
		}
	 ...
	 * target is null, or did not consume the event, it will be passed on to the
	 * most recently opened modal window if one exists (and the supplied target
	 * component was not a child of that window) and then to the default event
	 * targets if the event is still unconsumed.
	 */
	protected boolean dispatchEvent(BComponent target, BEvent event)
	{
		...
		return false;
		// System.out.println("target BrootNode"+target+" source
		// "+event.getSource());
	}
	...
	protected void setFocus(BComponent focus)
	{
		...
		if (_focus != focus)
		{
			if (_focus != null)
			{
				_focus.dispatchEvent(new FocusEvent(this, _tickStamp, FocusEvent.FOCUS_LOST));
			}
			_focus = focus;
			if (_focus != null)
			{
				_focus.dispatchEvent(new FocusEvent(this, _tickStamp, FocusEvent.FOCUS_GAINED));
			}
		}
	}
	...
	/**
	 * Recomputes the component over which the mouse is hovering, generating
	 * mouse exit and entry events as necessary.
	 */
	protected void updateHoverComponent(int mx, int my)
	{
		...
		if (_hcomponent != nhcomponent)
		{
			...
			if (_hcomponent != null)
			{
				_hcomponent.dispatchEvent(new MouseEvent(this, _tickStamp, _modifiers, MouseEvent.MOUSE_EXITED, mx, my));
			}
			// inform the new component that the mouse has entered
			if (nhcomponent != null)
			{
				nhcomponent.dispatchEvent(new MouseEvent(this, _tickStamp, _modifiers, MouseEvent.MOUSE_ENTERED, mx, my));
			}
