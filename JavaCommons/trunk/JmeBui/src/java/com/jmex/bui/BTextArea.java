 ...
 * (changing of color, toggling bold, italic and underline). Newline characters
 * in the appended text will result in line breaks in the on-screen layout.
 */
public class BTextArea extends BContainer
{
	...
	/**
	 * Returns a model that can be wired to a scroll bar to allow scrolling up
	 * and down through the lines in this text area.
	 */
	...
	/**
	 * Returns a text factory suitable for creating text in the style defined by
	...
	protected void configureStyle(BStyleSheet style)
	{
		...
		for (int ii = 0; ii < getStateCount(); ii++)
		{
			valigns[ii] = style.getVerticalAlignment(this, getStatePseudoClass(ii));
		}
	...
	protected void refigureContents(int width)
	{
		...
		for (int ii = 0, ll = _runs.size(); ii < ll; ii++)
		{
			...
			while ((offset = current.addRun(getTextFactory(), run, color, maxWidth, offset)) > 0)
			{
				_lines.add(current = new Line());
			}
			if (run.endsLine)
	...
	/**
	 * Called when our model has changed (due to scrolling by a scroll bar or a
	 * call to {@link #scrollToLine}, etc.).
	 */
	...
	protected static class Line
	{
		...
		/**
		 * Adds the supplied run to the line using the supplied text factory,
		 * returns the offset into the run that must be appeneded to a new line
		 * or -1 if the entire run was appended.
		 */
		public int addRun(BTextFactory tfact, Run run, ColorRGBA color, int maxWidth, int offset)
		{
			if (dx == 0)
			{
				start = run;
		...
		public void render(Renderer renderer, int x, int y, float alpha)
		{
			...
			for (int ii = 0, ll = segments.size(); ii < ll; ii++)
			{
				...
				text.render(renderer, dx1, y, alpha);
				dx1 += text.getSize().width;
			}
		}
